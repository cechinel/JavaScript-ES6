let Database = function() {
    ...
    function select(statement) {
        let parsedStatement = statement.match(/select (.*) from (.*)/),
            [undefined, columns, tableName] = parsedStatement;

        columns = columns.split(",");

        if (!(tableName in tables)) throw `The table ${tableName} does not exist`;
        ...
        for (let row of tables[tableName].data) {
            ...
            for (let column of columns) {
                column = column.trim();
                if (!(column in tables[tableName].columns)) throw `The column ${column} does not exist`;
