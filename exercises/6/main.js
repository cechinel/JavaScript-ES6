...
class Database {
    ...
    create(statement) {
        let parsedStatement = statement.match(/create table ([a-z]+) (\(.*\))/),
            [undefined, tableName, columns] = parsedStatement;
    ...
    insert(statement) {
        let parsedStatement = statement.match(/insert into ([a-z]+) (\(.*\)) values (\(.*\))/),
            [undefined, tableName, columns, values] = parsedStatement;
    ...
    select(statement) {
        let parsedStatement = statement.match(/select (.*) from ([a-z]+)(?: where (.*))?/),
            [undefined, columns, tableName, clauses] = parsedStatement;
...
console.log(JSON.stringify(database.execute("select id, name, age from author"), null, 2));
console.log(JSON.stringify(database.execute("select id, name, age from author where id = 1"), null, 2));
console.log(JSON.stringify(database.execute("select id, name, age from author where age < 60"), null, 2));
console.log(JSON.stringify(database.execute("select id, name, age from author where name = Linus Torvalds"), null, 2));
// returns
// [
//   {
//     "id": "1",
